...
class Box {
  ...
  public Box(Point center, double width, double height) {
    ...
    if (center == null)
      throw new IllegalArgumentException(
          "Null Point at center of Box"); // revent Boxes with null centers
    ...
    this.linesegs =
        new Lineseg[] {
          new Lineseg(this.verts[0], this.verts[1]),
  ...
  // Rotate the box around a point by angle theta in radians.  To rotate around the center, call
  // box.rotate(Math.pi/2, box.getCenter())
  ...
  public boolean contains(Point point) {
    ...
    if (Math.abs(levelbox.center.getX() - rotatedpoint.getX()) <= (levelbox.width / 2)
        && (Math.abs(levelbox.center.getY() - rotatedpoint.getY()) <= (levelbox.height / 2))) {
